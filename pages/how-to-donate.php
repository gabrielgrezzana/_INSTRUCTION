...
<div class="box-shadow style-right position-relative block-right-page content-block d-block c-block container p-3">
	<h5 class="heading">HOW TO DONATE</h5>
	<div id="accordion">	
	<?php
	$firstItem = true;
	...
	try {
		...
		$execute = $db->query("SELECT COUNT(*) FROM blog_posts WHERE postCat='how-to-donate' AND isDelete = 'false'");
		...
		$stmt = $db->query('SELECT * FROM blog_posts WHERE postCat="how-to-donate" AND isDelete = "false" ORDER BY postOrder DESC limit ' .$offset. ', ' .$limit. '');
		if ($total_results == 0) {
			?>
	...
	<?php if($pages > 1) { ?>
		<ul class="pagination">
		   ...
		   <li class='page-item <?php ($page <= 1 ? print 'disabled' : '')?>'>
			 <a title="<?= $config['website_title']; ?> &raquo; International Private Server" class='page-link' href='?do=how-to-donate&page=1'>First</a>
		   </li>
		   ...
		   <li class='page-item <?php ($i == $page ? print 'active' : '')?>'>
			 <a title="<?= $config['website_title']; ?> &raquo; International Private Server" class='page-link' href='?do=how-to-donate&page=<?php echo $i;?>'><?php echo $i;?></a>
		   </li>
		   <?php } ?>
		   ...
		   <li class='page-item <?php ($page >= $pages ? print 'disabled' : '')?>'>
			 <a title="<?= $config['website_title']; ?> &raquo; International Private Server" class='page-link' href='?do=how-to-donate&page=<?php echo $pages;?>'>Last                     
			 </a>
		   </li>
		 </ul>
	<?php }						
