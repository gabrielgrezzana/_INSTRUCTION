...
<div class="col-9 box-shadow clear-top">		
		...
		try {
			...
			$executeE = $db->query("SELECT COUNT(*) FROM blog_posts WHERE postCat='events'  AND isDelete = 'false'");
			$total_Ne = $executeNe->fetchColumn(); $total_E = $executeE->fetchColumn();
			$executeNe->closeCursor(); $executeE->closeCursor();
		...
		?>
			...
			<li role="presentation"><a title="<? if ($total_E > 0) { echo $total_E.' '; } ?><?= $config['website_title']; ?> Events" href="./events" role="tab">EVENTS <? if ($total_E > 0) { echo '<span class="notification">'. $total_E.'</span>'; } ?></a></li>
			<li role="presentation" class="active"><a title="<?= $config['website_title']; ?> Notice" href="./notice" role="tab">NOTICE</a></li>
		</ul>
		...
		<div class="tab-content">
			<div role="tabpanel" class="tab-pane active">
				...
				try {
					...
					$execute = $db->query("SELECT COUNT(*) FROM blog_posts WHERE postCat='notice' AND isDelete = 'false'");
					...
					$stmt = $db->query('SELECT * FROM blog_posts WHERE postCat="notice" AND isDelete = "false" ORDER BY postOrder DESC limit ' .$offset. ', ' .$limit. '');
					if ($total_results == 0) {
						?>
						<div class="panel panel-default">
							<div class="panel-heading">
								<h4 class="panel-title">
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collapseNotice">
										<h5 class="h2-accordion">No Data</h5>
									</a>
								</h4>
							</div>
							<div id="collapseNotice" class="a-color panel-collapse collapse <?php echo ($firstItem ? 'in show' : ''); ?>">
				...
				<center>
					<ul class="pagination">
					   ...
					   <li class='page-item <?php ($page <= 1 ? print 'disabled' : '')?>'>
						 <a title="First Page" class='page-link' href='/notice'>First</a>
					   </li>
					   ...
					   <li class='page-item <?php ($i == $page ? print 'active' : '')?>'>
						 <a title="Page <?php echo $i;?>" class='page-link' href='/notice/<?php echo $i;?>'><?php echo $i;?></a>
					   </li>
					   <?php } ?>
					   ...
					   <li class='page-item <?php ($page >= $pages ? print 'disabled' : '')?>'>
						 <a title="Last Page" class='page-link' href='/notice/<?php echo $pages;?>'>Last                     
						 </a>
					   </li>
					</ul>
				</center>
				...
				<center>
					<ul class="pagination">
					   ...
					   <li class='page-item active'>
						 <a title="Page 1" class='page-link' href='notice'>1</a>
					   </li>
					   <li class='page-item disabled'>
