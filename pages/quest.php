...
<div class="box-shadow style-right position-relative block-right-page content-block d-block c-block container p-3">
	<h5 class="heading">DAILY QUEST</h5>
	<div class="panel-group" id="accordion">
		<div id="nav-quest-accordion">
		<?php
		$firstItem = true;
		...
		try {
			...
			$execute = $db->query("SELECT COUNT(*) FROM blog_posts WHERE postCat='quest' AND isDelete = 'false'");
			...
			$stmt = $db->query('SELECT * FROM blog_posts WHERE postCat="quest" AND isDelete = "false" ORDER BY postOrder DESC limit ' .$offset. ', ' .$limit. '');
			if ($total_results == 0) {
				?>
				...
				<div class="panel panel-default">
					...
					<div id="collardion<?php echo $row['postID']; ?>" aria-labelledby="collardion<?php echo $row['postID']; ?>" data-parent="#nav-quest-accordion"  class="a-color panel-collapse collapse <?php echo ($firstItem ? 'show' : ''); ?>">
						<div class="panel-body">
