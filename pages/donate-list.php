...
<div class="box-shadow style-right position-relative block-right-page content-block d-block c-block container p-3">
	<h5 class="heading">DONATE LIST</h5>
	<div id="accordion">
	<?php
	$firstItem = true;
	...
	try {
		...
		$execute = $db->query("SELECT COUNT(*) FROM blog_posts WHERE postCat='donate-list' AND isDelete = 'false'");
		...
		$stmt = $db->query('SELECT * FROM blog_posts WHERE postCat="donate-list" AND isDelete = "false" ORDER BY postOrder DESC limit ' .$offset. ', ' .$limit. '');
		if ($total_results == 0) {
			?>
