...
async function DisplaySS() {
	...
	$.ajax({
		url: `https://stream.rfocp.net/@/stream/world`,
		dataType: 'JSON',
		async: false,
	...
	$(`[display-server='${wcx}'] .progress-bar.totalon`).css({ "width": `${data.UserNum / 10}%` }).attr("aria-valuenow", data.UserNum);
...
async function TopKill() {
	...
	$.ajax({
		url: `https://stream.rfocp.net/@/stream/topkill`,
		dataType: 'JSON',
		async: false,
		success: function (response) {
			data = response.data[wcx]
